...
- Schema 5 runtime attribute-store foundation is implemented and covered by focused lifecycle and operation tests: controller-owned INSTANCE values and `SceneTree`-owned CLASS values keyed by class ID; exact typed read/write/reset results; detached defaults; nullability; readonly/const rejection; scene continuity; and deterministic teardown. Schema 5 remains excluded from executable block paths. Call frames, inherited runtime resolution, accessors, visibility-aware external dispatch, UI, and Apply Constructor remain pending. See FOBJ-002/004/005 in the [Flujo Object Model contract](flow_object_model_contract.md).
