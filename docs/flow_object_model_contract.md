...
### Initial attribute-store implementation

FOBJ-002/004/005 map to `FlowAttributeRuntimeStore`, `FlowInstanceRuntimeStore`, `FlowClassRuntimeStore`, `FlowStoreResult`, and the schema 5 lifecycle branch in `PVController`. This delivery implements attribute values only, not requirement resolution, call frames, methods, or schema 5 execution.

Outside the editor, entering the tree initializes one INSTANCE store per controller and acquires the CLASS store by graph ID from metadata owned by that exact `SceneTree`. This is runtime metadata, not an autoload, singleton, static dictionary, or persistent graph field. The store has no back-reference to the tree, controller, or source Resource. A root `tree_exiting` connection closes each class store; ordinary current-scene replacement does not touch it. Exiting a controller closes its instance store and releases its class handle without resetting shared state. Re-entry creates fresh instance defaults and reacquires the existing class state. Retained closed handles reject operations.

Initialization validates schema 5, then snapshots enabled definitions by stable ID, preserving null collection slots as structural absence. Defaults and operation values are copied deeply for containers; the currently supported seven value types are value types, and Node/Object/Resource values are rejected by exact type checks. The copied metadata and defaults never follow later mutations of the source graph. Reacquiring a class ID with different active class-slot metadata/defaults fails with `store_definition_conflict` instead of resetting or overwriting live state.

These are storage primitives, not executable accessors. Inherited graphs require explicit project catalog context; absence returns `store_inheritance_unresolved`, rather than silently omitting inherited attributes. No API accepts a caller-controlled bypass for READONLY or CONST. Initialization from defaults is the only allowed initialization write in this delivery. Schema 5 never enters the schema 3 executors; schemas 1–4 keep their existing paths.

`read(attribute_id)`, `write(attribute_id, value)`, and `reset(attribute_id)` return a fresh `FlowStoreResult` with `ok`, `code`, `class_id`, `related_id`, `path`, and detached `value`. Acquisition additionally returns a transient store handle. Invalid graph initialization carries the original ordered validator diagnostics. The deterministic check order is closed/uninitialized store, missing ID (including disabled attributes), mutability, then nullability/type for writes. Successful operations use `ok`; errors use `store_closed`, `store_not_initialized`, `attribute_missing`, `attribute_const`, `attribute_readonly`, `attribute_null_not_allowed`, or `attribute_type_incompatible`. Reset restores the initialization snapshot and obeys the same mutability checks. Failed operations do not mutate live values or definitions; int-to-float coercion is not accepted. Initialization/acquisition failures additionally distinguish `store_already_initialized`, `store_schema_unsupported`, `store_definition_invalid`, `store_inheritance_unresolved`, `store_tree_unavailable`, `store_editor_context`, `store_registry_invalid`, and `store_definition_conflict`.

Focused evidence in `tests/runtime/flow_attribute_runtime_store_test.gd` proves controller isolation, class sharing by SceneTree and class ID, class continuity across scene changes and zero controllers, root teardown and retained handles, all seven types and nullability, mutation/reset rejection, same-ID definition conflicts, detached defaults, and absence of Node/Resource/Callable values in store data. General Model Mist, schema 5, and persistence regressions protect schemas 1–4 and persistent definitions. This runtime-only delivery has no visible representation and therefore requires no visual review.

### Inherited attribute resolution — validation handoff

`FlowRuntimeAttributeLayout.resolve(graph, catalog)` validates the existing portable catalog without retaining it or any loaded Resource, then snapshots the active graph and its ancestors in base-to-derived order. The supplied active definition is authoritative; ancestors use the catalog's existing UID/path resolver and diagnostics. Ten parent edges remain the maximum. Invalid catalogs, missing bases, cycles, excessive depth, name conflicts, or any repeated owned stable ID reject the entire layout before publishing slots. The global per-graph identity validator is unchanged. No locator, definition or invalid ID is repaired.

Slots carry transient `declaring_class_id`, visibility, type, nullability, mutability and deeply copied defaults, never new persistent fields. INSTANCE slots, including storage private to base declarers, live in the current controller's store. Each CLASS slot has exactly one value in its declaring class's SceneTree-owned store; derived handles route reads, writes and reset to that store. Acquisition preflights the entire hierarchy before registering anything or extending access context; conflicting live definitions or known parent relationships reject without altering existing values. Compatible successful acquisitions may extend the known class-ID parent map, never reset values. There is no static registry, retained Node/Resource, global catalog cache, executor expansion or name-based lookup.

Runtime callers can pass the project catalog to `FlowInstanceRuntimeStore.initialize(graph, catalog)` / `FlowClassRuntimeStore.acquire(tree, graph, catalog)`. For controller startup, `PVController.prepare_attribute_stores(catalog)` must run outside the editor and before tree entry; it retains only the detached layout. No catalog discovery or persistent controller field is added. A failed preparation blocks startup with its diagnostics rather than reusing an older successful snapshot. Re-entry uses fresh INSTANCE defaults from the prepared snapshot and existing CLASS state. Changing the graph's class ID requires a new preparation.

`read`, `write` and `reset` accept an optional final `requester_class_id`. Omission means the receiving store's class, never an inherited slot's declarer. Explicit context is supplied by trusted runtime integration; authentication through call frames is not implemented. PUBLIC accepts any valid class ID; PROTECTED requires the declarer or a descendant established by the validated parent map; PRIVATE requires the exact declarer. Checks run in order: lifecycle, slot existence, requester ID, visibility, mutability, nullability/type. Access failures use `attribute_requester_invalid` or `attribute_inaccessible`, preserve values, and cannot bypass READONLY/CONST. Successful results identify the slot's declaring class. Initialization carries ordered catalog/model diagnostics and reports the first error's code/path/related ID.

`tests/runtime/flow_inherited_attribute_store_test.gd` covers three-level inheritance, the symmetric visibility-name matrix, instance/class routing, reset, access/type rejection, collision, catalog, depth and lifecycle cases. Its focused validation passes together with Model Mist, schema 5 foundation, base stores, persistence, schema 3 runtime and editor regressions.
...
Attribute names are unique by exact case-sensitive comparison within each declaring class. Across the effective hierarchy, a same-name pair produces `inherited_attribute_conflict` only when both the inherited and derived attributes are non-PRIVATE (PUBLIC or PROTECTED), even with different IDs. FOBJ-009 provides the symmetric PRIVATE exception. Their IDs, not their names or positions, are references. Any repeated stable ID in the hierarchy is an error regardless of visibility. Ordered `null` positions remain structural absence and are not nullable values.
...
For attribute-name conflicts this exception is symmetric: base PRIVATE → derived PRIVATE/PUBLIC/PROTECTED is allowed, and base PUBLIC/PROTECTED → derived PRIVATE is allowed, always with distinct stable IDs. Base PUBLIC/PROTECTED → derived PUBLIC/PROTECTED remains rejected until explicit overrides are implemented. Keeping a base-private storage slot in a derived instance does not grant the derived class access to it; its `declaring_class_id` and access rules remain unchanged. This rule neither introduces overrides nor resolves by name.
...
3. **Runtime attribute stores (implemented):** add isolated INSTANCE and SceneTree/class-owned CLASS stores with no executor expansion. Tests prove controller isolation, class sharing, scene continuity, initialization, teardown, nullability, readonly, const, reset, type safety, detached defaults, and zero Resource mutation. Call-frame storage remains part of the later Methods and Programmer mode delivery.
