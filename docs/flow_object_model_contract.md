...
These are declaring-class storage primitives, not cross-class callable accessors. Runtime inheritance/catalog context and visibility-aware external access remain deferred: graphs with a base class explicitly return `store_inheritance_unresolved`, rather than silently omitting inherited attributes. No API accepts a caller-controlled bypass for READONLY or CONST. Initialization from defaults is the only allowed initialization write in this delivery. Schema 5 never enters the schema 3 executors; schemas 1–4 keep their existing paths.
