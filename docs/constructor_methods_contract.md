...
### First vertical delivery (CMRUN-001–006, implemented; visual review pending)

- **CMRUN-001:** reuse schema 3 without persistent changes or schema 4. Inspector order is Constructor, Processes, Timers, State Machines, Methods, Variables. The Inspector owns creation and stable-ID selection; configuration lives only in Flujo.
- **CMRUN-002:** the unique constructor cannot be added, removed, or moved. Its enabled blocks execute once immediately before Ready inside the controller's guarded startup, not Godot's native `_init`. Definitions remain immutable.
- **CMRUN-003:** Methods use independent first-available names `Flujo`, `Flujo 1`, etc. Existing method-name validation remains in force. Method authoring supports name, Enabled and ordered Print/Everything Flows blocks through scene Undo/Redo, including block names and F2/Enter/Escape.
- **CMRUN-004:** Call Method authoring and execution are available only in Processes and Timers. Targets use the existing `method_id`, never names. Renaming preserves calls. Deleting a method preserves its callers' IDs for diagnostics and is undoable.
- **CMRUN-005:** missing, invalid, ambiguous or disabled targets do not stop subsequent blocks. No method-to-method calls, recursion, argument transport or return execution is introduced. Methods declaring parameters or a return are diagnosed and skipped. Previously persistable calls in other containers remain persistable but are not executable in this delivery.
- **CMRUN-006:** reuse the runtime output boundary and existing signals; method output additionally exposes entry-container, method and call IDs through a contextual signal. Constructor output identifies its own container. Focal runtime/editor tests and the general regression suite provide automated evidence; visual approval remains separate.

This document defines schema 3 behavior for Constructor declarations and reusable methods. Its persistent structural foundation, method-call references, optional typed return definitions and the limited vertical delivery above are implemented. General runtime state, scene bindings, argument bindings, return execution and call-cycle validation remain future work. Automated focal and general regression evidence is complete; visual validation of the vertical delivery remains pending.
...
`FlowConstructorDefinition` is one persistent `FlowBlockContainer` embedded in each schema 3 `FlowGraph`. Its enabled starter blocks run in the guarded controller startup immediately before Ready. This controlled phase is not native Godot `_init` and does not resolve scene dependencies.
...
Methods remain immutable definitions. The first vertical delivery executes only their enabled starter blocks from Ready/Timer calls. Their local values, argument values, return behavior, and general activation frames belong to the future runtime context.
...
- **MCALL-007 — Deferred semantics:** CMRUN-001–006 implement limited Ready/Timer call execution and authoring. Argument bindings, return blocks and runtime return values, parameter bindings, recursion, cycle detection and scene bindings remain deferred. Other persistable call locations do not acquire execution semantics.
...
Recursion and call-cycle validation are not implemented in the persistent method-call foundation. Direct self-calls and indirect cycles are preserved by model validation. The first runtime delivery diagnoses and skips every call inside a method, preventing recursive execution without changing those definitions.
